...
{
    // --- CUSTOM MOD: Intercept hardcoded damage ---
            ...
            if (jsonData.contains("spells")) {
                ...
                for (int i = 0; i < Spell::SPELL_COUNT; ++i) {
                    ...
                    if (spls.contains(id)) {
                        ...
                        // New Movement and Extra Value Hooks
                ...
                // Auto-generate default spell stats
                for (int i = 0; i < Spell::SPELL_COUNT; ++i) {
                    std::string id = std::to_string(i);
                    jsonData["spells"][id] = {
                        {"name", std::string(spells[i].name)},
