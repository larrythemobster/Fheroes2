...
fheroes2::GameMode Game::SelectCampaignScenario( const fheroes2::GameMode prevMode, const bool allowToRestart )
{
    ...
    while ( le.HandleEvents() ) {
        ...
        if ( le.isMouseRightButtonPressedInArea( buttonCancel.area() ) ) {
            ...
            const bool isResurrectionMap = ( mapInfo.version == GameVersion::RESURRECTION );
            const bool isMapLoaded = isResurrectionMap ? world.loadResurrectionMap( mapInfo.filename ) : world.LoadMapMP2( mapInfo.filename, isSWCampaign );
